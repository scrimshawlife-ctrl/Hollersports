...
- Free-first **multi-ingest auto-compete** merges candidates across all INGESTED events (not first-only)
...
- **Track F ML pipeline:** features → L2 logistic (optional sklearn HGB) → temperature ensemble → EV annotate feeding `MODEL_PROBABILITY_EDGE`; free-first odds movement; offline sentiment; model cards; axial **stub** + optional **PyTorch axial** (`[torch]` / `make ml-axial-train`, `POST /v1/ml/axial`); gated retrain-apply
