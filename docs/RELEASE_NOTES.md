...
- **Track F ML pipeline:** features → L2 logistic (optional sklearn HGB) → temperature ensemble → EV annotate feeding `MODEL_PROBABILITY_EDGE` (`make ml-e2e` / `make ml-compete`; API `/v1/ml/*` incl. retrain-check; Health Workbench; offline sentiment lexicon; free-first odds movement; stdlib default)
